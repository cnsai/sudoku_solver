...
# find the countours
# hierarchy, contours0, hierarchy = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
...
for i in range(SUDOKU_SIZE):
    for j in range(SUDOKU_SIZE):
        ...
        axarr[i, j].imshow(
            cv2.resize(sudoku[i * 9 + j, :].reshape(IMAGE_WIDHT, IMAGE_HEIGHT), (IMAGE_WIDHT * 2, IMAGE_HEIGHT * 2)),
            cmap=pl.gray())
        axarr[i, j].axis("off")
...
print("===== Finish KNN, Start draw KNN result =====")

with open("sudoku_input.txt") as ansFile:
...
print("===== Finished, Please execute C++ Sudoku_Solver =====")
